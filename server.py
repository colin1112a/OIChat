...
Chinese:
1. 修复在没有配置文件时，服务端生成的配置文件是 `config.txt` 的问题。

English:
1. Fix the issue where the configuration file generated by the server is `config.txt` when there is no configuration file.
"""
...
if __name__ == "__main__":
    ...
    try:
        ...
        json_dict = {"start_Port" : porttmp, "admin_Name" : op, "admin_Password" : oppassword, "version_Verification" : True, "password_Login" : False, "login_Password" : "123456"}
        config_json = json.dumps(json_dict)
